...
const DEFAULT_EXCLUDE = ['MYSQL_PASSWORD', 'TC_KEY', 'SECRET', 'PASSWORD', 'TOKEN'];
...
<script>
  ...
  window.__RUNTIME_ENV__ = window.__RUNTIME_ENV__ || {};
      ...
      return [{ tag: 'script', children: envScript, injectTo: 'head' }];
    },
  };
}
...
/** 过滤敏感环境变量 */
function filterEnvVars(vars: Record<string, string>, options: { exclude: string[] }): Record<string, string> {
  const result: Record<string, string> = {};

  for (const [key, value] of Object.entries(vars)) {
    const shouldExclude = options.exclude.some((pattern) => key.toUpperCase().includes(pattern.toUpperCase()));
    if (!shouldExclude) {
      result[key] = value;
    }
  }
